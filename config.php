...
function ensure_schema_updates($conn) {
    ...
    foreach ($utf8mb4Tables as $t) {
        if (table_exists($conn, $t)) {
            ...
            $charsetRes = @mysqli_query($conn, "SELECT CCSA.character_set_name AS csname FROM information_schema.TABLES T INNER JOIN information_schema.COLLATION_CHARACTER_SET_APPLICABILITY CCSA ON T.TABLE_COLLATION = CCSA.collation_name WHERE T.TABLE_SCHEMA = DATABASE() AND T.TABLE_NAME = '{$tEsc}'");
            $charsetRow = $charsetRes ? mysqli_fetch_assoc($charsetRes) : null;
            $currentCharset = ($charsetRow && isset($charsetRow['csname'])) ? (string)$charsetRow['csname'] : '';
